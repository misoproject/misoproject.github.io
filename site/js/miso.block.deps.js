/**
* Miso.Block - v0.0.1 - 11/5/2012
* http://github.com/misoproject/block
* Copyright (c) 2012 Alex Graul, Irene Ros, Rich Harris;
* Dual Licensed: MIT, GPL
* https://github.com/misoproject/block/blob/master/LICENSE-MIT 
* https://github.com/misoproject/block/blob/master/LICENSE-GPL 
*/
...
/**
* Miso.Storyboard - v0.0.1 - 11/5/2012
...
(function(global, _, $) {
  ...
  /**
  * Miso Block is a visual wrapper for visual content.
  * It can contain a storyboard, has event bindings, an element and the ability
  * to maintain a style dictionary.
  * Options:
  *   storyboard - a Miso.Storyboard object or the options required to create one
  *   data - a generic pointer to a set of data associated with the block. Could be a Miso Dataset.
  *   style - a Miso.Style object or the options required to create one. 
  *   el - An element or selector to fetch an element from the dom.
  *   template - a path to a template file
  *   mixins - an array of Mixin objects that get mixed into the block
  */
  Miso.Block = function(options) {
    options = options || {};

    // initialize storyboard if one is provided. One can be
    // an instance of a storyboard, or simply the correct
    // object structure that would be used to initialize one.
    if (options.storyboard) {
      var storyboard = options.storyboard;
      if ( storyboard instanceof Miso.Storyboard ) {
        storyboard._context = this;
      } else {
        storyboard.context = storyboard.context || this;
        storyboard = new Miso.Storyboard(storyboard);
      }
    ...
    // unique id
    this._id = _.uniqueId();
    // transformed data for the purposes of this visualization
    ...
    // set up style object
    // if we already have one from the parent, clone it.
    // otherwise create a new one.
    if (typeof this.style === "undefined") {
      this.style = new Miso.Style({}, this);
    } else if (this.style instanceof Miso.Style) {
      this.style = this.style.clone();
      this.style.block = this;
    } else {
      this.style = new Miso.Style(this.style, this);
    }

    if (options.style) {
      this.style.extend(options.style);
    }

    // set up elemenet
    this.setElement(options.el, options);

    // save template, if one is defined.
    if (options.template || this.template) {
      // cache templates
      if (!Miso.Templates.has(options.template || this.template)) {
        Miso.Templates.add(options.template || this.template);
      }
      this.template = Miso.Templates.get(options.template || this.template);
    }

    // if there are any mixins, add them
    this.mixins = this.mixins || [];
    if (options.mixins) {
      this.mixins = this.mixins.concat(options.mixins);
    }
  ...
  /**
  * Allows one to define a block constructor like so:
  * Miso.Block.extend({
  *   //...
  * })
  * All properties will be copied over. styles will be cloned when
  * extending an already existing block.
  */
  Miso.Block.extend = function(options) {
    options = options || {};
    ...
    if (parent.prototype.style && options.style) {
      var newStyle;
      if (parent.prototype.style instanceof Miso.Style) {
        newStyle = parent.prototype.style.clone().extend(options.style);
      } else {
        newStyle = new Miso.Style(parent.prototype.style).extend(options.style);
      }
      child.prototype.style = newStyle;
    }

    _.each(child.prototype, function(f, name) {
      child.prototype[name] = f;
    });
    child.extend = Miso.Block.extend;
    return child;
  };

  // Passthrough. extend === define.
  Miso.Block.define = Miso.Block.extend;

  _.extend(Miso.Block.prototype, Miso.Events, {

    extend : Miso.Block.extend,
    
    /**
    * Default passthrough initialize method.
    ...
    */
    ...
    /** 
    * Mixes in a Mixin object containing methods that shold
    * be available on a block.
    * Params:
    *   MixinClasses - Single or Array of Mixin objects. (Note, not instances.)
    */
    mixin : function(MixinClasses) {

      if (!_.isArray(MixinClasses)) {
        MixinClasses = [MixinClasses];
      }

      _.each(MixinClasses, function(MixinClass) {
        // if we have instances from an extension, then mix in
        // their constructor on this block.
        if (MixinClass instanceof Miso.Block.Mixin) {
          MixinClass = MixinClass.constructor;
        }

        // initialize new mixin (this also takes care of the
        // wiring inside the mixin itself.)
        var mixin = new MixinClass(this);
      }, this);
    ...
    /**
    * Sets the element associcated with the block
    * Params:
    *   el - selector or element
    *   options - an object that can contain dimensions
    *             such as: height, width
    *   
    */
    setElement : function(el, options) {
      this._findElement(el, options);
    },
    ...
    _setDimensions : function(options) {
      options = options || {};
      var __sizeProperties = ["height", "width"];

      _.each(__sizeProperties, function(prop) {
        if (typeof options[prop] !== "undefined") {
          this.el.css(prop, options[prop]);
          this[prop] = options[prop];
        } else {
          if (typeof this.el !== "undefined") {
            this[prop] = this.el[prop].call(this.el);
          }
        }
      }, this);
    },

    // some logic borrowed from Backbone.js 0.9.2 Views.
    // (c) 2010-2012 Jeremy Ashkenas, DocumentCloud Inc.
    _findElement : function(selector, options) {
      ...
      if (typeof selector === "string" ) {
        ...
        // this is an object, with potentially the following attributes:
        var nodeType = selector.type || "div",
            className = selector.className || undefined;
...
(function(global, _, $) {
  ...
  /**
  * A constructor for creating a new mixin. A mixin is just a collection
  * of functionality that is not defined by default on a blUsually just
  * call block's .mixin(MyMixinConstructor) method.
  */
  Miso.Block.Mixin = function(block) {

    // save block
    if (typeof block === "undefined") {
      throw new Error("Block is required for mixin!");
    }
    
    this.block = block;

    // get all functions and attach them to the block. Exclude the following
    // few functions:
    var blacklist = ['initialize', 'style', 'wiring'];
    _.each(this.constructor.prototype, function(f, name) {
      if (blacklist.indexOf(name) === -1) {
        block[name] = f;
      }
    }, this);

    // subscribe any of the mixin functions to the block's events.
    if (this.wiring) {
      _.each(this.wiring, function(method, trigger) {
        if (typeof this[method] !== "undefined") {
          block.subscribe(trigger, this.block[method], this);
        } else {
          throw new Error("The method " + method + " doesn't exist on this mixin.");
        }
      }, this);
    }

    // run any initialization sequence we might have
    _.bind(this.initialize, block);
    this.initialize();
  };

  /**
  * Creates a mixin for a block that contains a single set of functionality
  * Options - object of keys and functions as values. These
  *           functions will be attached to the block they are
  *           extending. There are a few special functions you can set
  *              initialize - a function that will run when the Mixin is
  *                       initialized. The context is the block.
  *              wiring : An object containing key value pairs of the block
  *                       triggers that then map to one of the mixin functions.
  *                       eg.: { 'focus.start' : 'highlight' }
  */
  Miso.Block.Mixin.extend = function(options) {
    options = options || {};
    ...
    child.extend = Miso.Block.Mixin.extend;

    return child;
  };

  Miso.Block.Mixin.define = Miso.Block.Mixin.extend;

  Miso.Block.Mixin.prototype.initialize = function() {};
...
(function(global) {
  ...
  * Params:
  *   props - the object containing style properties and their values.
  *   block - Optional. The block to attach to this style.
  */
  Miso.Style = function(props, block) {
    this._props = props || {};
    this.block = block  || null;
  };

  Miso.Style.inherits = function(name) {
    return function() {
      var args = [name];
  ...
  _.extend(Miso.Style.prototype, {

    /**
    * Allows extending of current style with new style properties
    * Params:
    *   1..* - object of properties, for example: { color : 'red' }
    *          or another miso.style instance.
    */
    extend : function() {
      if (arguments.length === 0) {
        throw new Error("You must provide one or more objects " +
                        "with style properties or another style.");
      }

      var props = [{}, this._props].concat(
        _.map(arguments, function(obj) {
          return (obj instanceof Miso.Style ? obj._props : obj);
        })
      );
      this._props = _.extend.apply(this, props);
      return this;
    },

    /**
    * Sets the block for which this style is attached to.
    * Params:
    *   block
    */
    ...
    /**
    * Returns a clone of the current style object
    */
    clone : function() {
      return new Miso.Style(_.clone(this._props));
    },

     /**
    * Returns the value of a property. If that value
    * is a function, it will call that function first and
    * return its value.
    * Properties:
    *   name - property name
    *   ...* - Optional arguments to pass to the property value if it's
    *          a function.
    * Returns:
    *   Value
    */
    get : function(name) {
      ...
      // is our property a function?
      if ( _.isFunction(prop) ) {

        // extract potential arguments and apply them to function
        var args = [];
...
(function(global, _, $) {
  ...
  var _buildTemplate = function(path) {

    if (!Miso.Templates.has(path)) {
      $.ajax({ 
        url: Miso.Templates.root + path + '.html', 
        async: false, 
        success : function(contents) {
          Miso.Templates._cache[path] = Miso.Templates.builder(contents);
        },
        error : function() {
          throw "This template doesn't exist!";
        }
      });
    }
  };
  ...
  /**
  * A singleton cache and utility set for managing templates.
  */
  Miso.Templates = {
    _cache : {},
 
    // The root folder underwhich templates resie
    root : 'templates',

    // The template builder.
    builder : _.template,

    // The rendering itself. takes a rendinging function
    // and the data to be passed to it
    renderer : function(template, data) {
      return template(data);
    },

    // returns true if there is a template in the cache
    has : function(path) {
      return (typeof this._cache[path] !== "undefined");
    },

    // adds a new template to the cache by fetching it from the
    // path specified and then caching it.
    add : function(path) {
      return _buildTemplate(path);
    },

    // retrieves a template from the cache. If params are provided
    // then the template is compiled, otherwise the template function
    // that was cached is returned.
    get : function(path, params) {
      if (!this.has(path)) {
        throw "Template doesn't exist. Add it first.";
      } else {
        if (typeof params === "undefined") {
          return this._cache[path];  
        } else {
          return this.renderer(this._cache[path], params);
        }
      }
    }
  };
